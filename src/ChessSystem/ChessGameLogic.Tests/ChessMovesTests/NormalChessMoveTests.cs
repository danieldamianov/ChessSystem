namespace ChessGameLogic.Tests.ChessMovesTests
{
    ...
    public class NormalChessMoveTests
    {
                ...
                , char targetPositionHorizontal, int targetPositionVertical, bool isValid)
        {
            ...
            if (isValid == false)
            {
                Assert.That(() => normalChessMoveType.GetConstructors(BindingFlags.Instance
                    | BindingFlags.NonPublic)[0].Invoke(new object[] { initialPositionHorizontal,initialPositionVertical
                , targetPositionHorizontal, targetPositionVertical }),
            ...
            else
            {
                ...
                chessBoardPositionType.GetConstructors(BindingFlags.Instance
                    | BindingFlags.NonPublic)[0].Invoke(new object[] {initialPositionHorizontal,initialPositionVertical })}));

                Assert.That((bool)chessBoardPositionEqualsMethod.Invoke(targetPosition, new object[] {
                chessBoardPositionType.GetConstructors(BindingFlags.Instance
                    | BindingFlags.NonPublic)[0].Invoke(new object[] { targetPositionHorizontal, targetPositionVertical })}));
            }
        }
